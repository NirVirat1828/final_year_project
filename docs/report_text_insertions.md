# Report Text Insertions — Ready to Paste

Below is every piece of text, code, caption, and table you need to insert into the existing report. Each section is labeled with the **exact page number** and **section heading** from your current report. The writing style matches your existing report's formal academic tone.

---
---

## INSERT 1 — Page 13 (after "Stage 6: Inference & Deployment")

> Your existing text ends with: *"A high-performance REST API is used to provide the completed models. The system can support industrial sorting..."*

**Append the following paragraph directly after the existing Stage 6 text:**
...
## INSERT 2 — Page 14 (Replace Fig 2.1) and Page 17 (Replace Fig 2.2)

> Your existing figures show a linear flow from Sensor → Preprocessing → Models → API. Replace or supplement with the updated architecture below.
...
**Mermaid diagram code (render using mermaid.live or any Mermaid-compatible tool):**
...
graph TD
    ...
    subgraph Backend["FastAPI Backend (Python 3.13)"]
        Preproc["StandardScaler Preprocessing"]
...
## INSERT 3 — Page 16 (after "Stage 4: Modeling & Benchmarking")

> Your existing text describes the four modeling tracks. Add the following paragraph after the Stacking Ensemble bullet point to introduce the Decision Engine:
...
## INSERT 4 — Page 31 (Table 2.16: Python Libraries)

> Your existing table lists NumPy, Pandas, Scikit-learn, etc. Add the following rows to reflect the new web application stack:

**New rows to add to Table 2.16:**
...
## INSERT 5 — Page 33 (Deployment Requirements Table)

> Your existing table shows Edge Deployment and On-Premise Server. Add a new row:

**New row for the Deployment Requirements table:**
...
## INSERT 6 — Page 35 (Table 3.2: Integration Test Cases)

> Your existing table has IT-01 through IT-05. Add the following rows:

**New rows for Table 3.2:**
...
## INSERT 7 — Pages 35–36 (New Subsection: "3.4 Frontend Implementation")

> Insert this entire new subsection after the existing Section 3.3 content.
...
## INSERT 8 — Page 37 (Section 4.1: Results)

> Your existing text shows an inference log for SVR. Below it, add a new subsection showing the integrated Decision Engine output:
...
## INSERT 9 — Page 38 (after "Integrated Shelf Life Report" / before Figure 4.2)

> Your existing text discusses folic acid prediction and grade sensitivity. Add this paragraph to introduce the Decision Engine's role in the shelf life report:
...
## INSERT 10 — Page 70 (Chapter 9: Discussion — New Subsection)

> Insert this as a new subsection within Chapter 9, after the existing discussion of verification plans. Title it "9.2 Master System Architecture Comparison with Existing Research"
...
## INSERT 11 — Page 74 (Replace "9.1.2 Recommended Production Output Format")

> Your existing text shows an ASCII-art "Proposed Output Card". Replace or supplement it with the following:
...
## INSERT 12 — Page 80 (Appendix: Replace/Update Code Snippets)

> Your existing Appendix starts with A.1 (Data Loading). Add a new appendix section **A.10** for the Decision Engine and **A.11** for the WebSocket endpoint:
...
class DecisionEngine:
    ...
    def generate_decision(self, predicted_class, prediction_probabilities,
        ...
        final_shelf_life = max(0, base_days)

        # Phase 5: Business Risk Formulation (cascading rules)
        risk = "Low"
        ...
        if confidence_level == "Low" and risk == "Low":
            risk = "Medium"  # Escalate uncertain premium products
        ...
        return BusinessDecision(
            ...
            business_risk=risk,
            estimated_shelf_life=f"{final_shelf_life} days",
            reasoning=reasoning
        )
```
...
async def websocket_live_predict(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Run full ML pipeline on each frame
...
export async function getDataset(page = 1, pageSize = 50) {
  const response = await fetch(
    `${BASE_URL}/dataset?page=${page}&page_size=${pageSize}`
  );
...
export async function getSimulatedScan() {
  const response = await fetch(`${BASE_URL}/hardware/simulate`);
...
## SUMMARY: Complete Edit Checklist

| # | Page | Section / Location | Type | Action |
|---|------|--------------------|------|--------|
| 1 | 13 | Stage 6: Inference & Deployment | Text | Append 3 paragraphs about full-stack architecture |
| 2 | 14, 17 | Fig 2.1 / Fig 2.2 | Diagram | Replace with new Master System Architecture |
| 3 | 16 | After Stage 4 (Modeling) | Text | Insert Stage 5b: Decision Engine Integration |
| 4 | 31 | Table 2.16: Python Libraries | Table | Add 10 new library rows |
| 5 | 33 | Deployment Requirements | Table | Add 1 new deployment scenario row |
| 6 | 35 | Table 3.2: Integration Tests | Table | Add 4 new test case rows (IT-06 to IT-09) |
| 7 | 35–36 | New Section 3.4 | Text + Figs | Insert entire Frontend Implementation section with 5 screenshots |
| 8 | 37 | Section 4.1: Results | Code Block | Insert Decision Engine output examples |
| 9 | 38 | Integrated Shelf Life Report | Text | Insert paragraph about penalty stacking |
| 10 | 70 | Chapter 9: Discussion | Table + Text | Insert architecture comparison table with 6 papers |
| 11 | 74 | Section 9.1.2 | Text | Replace ASCII card with Dashboard implementation description |
| 12 | 80+ | Appendix | Code | Add A.10 (Decision Engine), A.11 (WebSocket), A.12 (Frontend API) |
